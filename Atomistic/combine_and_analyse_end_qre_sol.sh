...
rm DSSP_averages*txt
        for (( i=0; i<${#ARRAY[@]}; i++ )); do
                for j in {0..4}; do
                        SYS=${NUC[k]}.${ARRAY[i]}
                        TPR=../${NUC[k]}/${SYS}_md_${j}.tpr
                        echo -e r1270 '\n' r1273-1275 '\n' q | $GMX51 make_ndx -f ${TPR} -o subs_${j}.ndx >& ndx_${ARRAY[i]}_${j}
                        echo  r_1270 | $GMX51 do_dssp -s ${TPR} -f ../${NUC[k]}/${SYS}_md_${j}.xtc -sc ${SYS}_${j}.dssp.contact1.xvg -n subs_${j} -nice -19 >& dssp1_${ARRAY[i]}_${j}
                        echo  r_1273-1275 | $GMX51 do_dssp -s ${TPR} -f ../${NUC[k]}/${SYS}_md_${j}.xtc -sc ${SYS}_${j}.dssp.contact2.xvg -n subs_${j} -nice -19 >& dssp2_${ARRAY[i]}_${j}
                        C=(contact1 contact2)
                        for (( c=0; c<${#C[@]}; c++ )); do
                                ave1=0
                                ave2=0
