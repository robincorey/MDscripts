...
############################
prot=2oar		#### >>> Needs to match input PDB
...
z=13			#### >>> Simulation box z axis. 15 as standard
...
if [[ "$1" == "-h" ]]; then echo "
 1;; Generate topology file from pdb"; sleep 0.2; echo "
...
if  [[ $propka == "yes" ]]; then 
	sh /home/birac/Desktop/MD_KIT/DO_PROPKA.sh ${prot}.pdb
	cp ${prot}_postpdb2gmx.gro ${prot}.gro	 	
elif [[ $propka == "no" ]]; then
	if [[ "$ters" == "yes" ]]
		then echo -e 3 '\n' 3 | pdb2gmx -f ${prot}.pdb -o ${prot}.gro -p ${prot}.top -ff $ff -water ${sol} -ignh -ter   	
	else
		pdb2gmx -f ${prot}.pdb -o ${prot}.gro -p ${prot}.top -ff $ff -water ${sol} -ignh 
	fi
else
	echo didnt read propka option
	exit 0
fi
