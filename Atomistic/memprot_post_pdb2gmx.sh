...
############################
pro=$1
prot=${pro%.*}		#### >>> Needs to match input PDB
nuc=ATP			#### >>> ADP or ATP (if none leave blank)
forcefield=gromos      	#### >>> Either opls* or gromos** at the moment
gromoslipidonly=pope	#### >>> POPE, POPC and POPG all working
...
if [[ "$forcefield" == "opls" ]]
	then cp $MM/popc_opls.itp . 
	cp $MM/512-popc-eq.pdb .
	cp $MM/MDPs/*mdp .
	cp $MM/membed.dat .
fi
...
if [[ "$forcefield" == "gromos" ]]
	then cp $MM/ITPs/*itp . 
	cp $MM/512-$lipid-gromos53a6.pdb .
	cp $MM/MDPs/*mdp .
	cp $MM/membed.dat .
fi
...
gmx_sse editconf -f ${prot}.gro -o ${prot}.gro.pdb

ReportError ${prot}.gro.pdb
...
if [ "$forcefield" == "gromos" ] && [ "$lipid" == "pope" ]
        then gmx_sse editconf -f protmem.gro.pdb -o ${prot}-mem.pdb.gro -c -box 12.2528 12.3878 $z -bt triclinic
elif [ "$forcefield" == "gromos" ] && [ "$lipid" == "popg" ]
	then gmx_sse editconf -f protmem.gro.pdb -o ${prot}-mem.pdb.gro -c -box 13.2756 13.2546 $z -bt triclinic
elif [ "$forcefield" == "gromos" ] && [ "$lipid" == "popc" ]
        then gmx_sse editconf -f protmem.gro.pdb -o ${prot}-mem.pdb.gro -c -box 13.3356 13.3040 $z -bt triclinic
elif [[ "$forcefield" == "opls" ]]
	then gmx_sse editconf -f protmem.gro.pdb -o ${prot}-mem.pdb.gro -c -box 13.1708 13.1708 $z -bt triclinic
fi

ReportError ${prot}-mem.pdb.gro
...
if [ "$forcefield" == "gromos" ] && [ "$lipid" == "pope" ]
	then 
	sed '/pope_GROMOS-CKP.itp/d' ${prot}.top -i
	sed '/Include water topology/i \
# include "'pope_GROMOS-CKP.itp'"\ ' ${prot}.top -i
elif [ "$forcefield" == "gromos" ] && [ "$lipid" == "popg" ]
        then 
	sed '/dpopgGromacs4_53a6.itp/d' ${prot}.top -i
	sed '/lpopgGromacs4_53a6.itp/d' ${prot}.top -i
	sed '/Include water topology/i \
...
' ${prot}.top -i
	...
	then 
	sed '/popcGromacs4_53a6.itp/d' ${prot}.top -i
	sed '/Include water topology/i \
# include "'popcGromacs4_53a6.itp'"\ ' ${prot}.top -i
elif [[ "$forcefield" == "opls" ]]
	then 
	sed '/popc_opls.itp/d' ${prot}.top -i
	sed '/Include water topology/i \
...
if [[ "$forcefield" == "opls" ]]
	then gmx_sse genbox -cp protmem.gro -o solmem.gro -p ${prot}.top -cs
fi
...
gmx_sse editconf -f premembed.gro -o premembed.pdb

ReportError premembed.pdb
...
gmx_sse editconf -f protein.pdb -o protein.gro
gmx_sse editconf -f Memx.pdb -o Memx.gro
gmx_sse editconf -f SOLx.pdb -o SOLx.gro
gmx_sse editconf -f NAx.pdb -o NAx.gro
gmx_sse editconf -f CLx.pdb -o CLx.gro
gmx_sse editconf -f Nucx.pdb -o Nucx.gro
gmx_sse editconf -f MGx.pdb -o MGx.gro
...
gmx_sse editconf -f all.pdb -o all.gro
...
echo q | gmx_sse make_ndx -f solmem.gro -o index > ndx
...
if [ "$forcefield" == "gromos" ] && [ "$lipid" == "popg" ]
        then echo -e '"DPO"' '|' '"LPO"' '\n' '"MG"' '|' '"'$nuc'"' '|' '"Protein"' '\n' name $memno Mem '\n' name $nucno Nuc '\n'q  | gmx_sse make_ndx -f solmem.gro -o memsolpre.ndx -n index.ndx
elif [ "$forcefield" == "gromos" ] && [ "$lipid" == "pope" ]
        then echo -e '"POP"' '\n' '"MG"' '|' '"'$nuc'"' '|' '"Protein"' '\n' name $memno Mem '\n' name $nucno Nuc '\n'q  | gmx_sse make_ndx -f solmem.gro -o memsolpre.ndx -n index.ndx
elif [ "$forcefield" == "gromos" ] && [ "$lipid" == "popc" ]
        then echo -e '"POP"' '\n' '"MG"' '|' '"'$nuc'"' '|' '"Protein"' '\n' name $memno Mem '\n' name $nucno Nuc '\n'q  | gmx_sse make_ndx -f solmem.gro -o memsolpre.ndx -n index.ndx
...
if [ "$forcefield" == "gromos" ] && [ "$lipid" == "popg" ]
	then echo -e '"Mem"' '|' '"Water_and_ions"' '\n'q | gmx_sse make_ndx -f solmem.gro -n memsolpre.ndx -o memsol.ndx
...
if [[ ! -z "${nuc}" ]]; then
gmx_sse grompp -v -f gmembed_nuc.mdp -o gmembed -c solmem.gro -p ${prot}.top -n memsol.ndx -maxwarn 4
fi

if [[ -z "${nuc}" ]]; then
gmx_sse grompp -v -f gmembed.mdp -o gmembed -c solmem.gro -p ${prot}.top -n memsol.ndx -maxwarn 4
fi
...
if [[ ! -z "${nuc}" ]]; then
echo Nuc Mem | gmx_sse mdrun -s gmembed.tpr -membed membed.dat -o traj.trr -c membedded.gro -e ener.edr -cpt -1 -v -stepout 100 -mn memsol
elif [[ -z "${nuc}" ]]; then
echo Protein Mem | gmx_sse mdrun -s gmembed.tpr -membed membed.dat -o traj.trr -c membedded.gro -e ener.edr -cpt -1 -v -stepout 100 -mn memsol
fi
...
gmx_sse grompp -v -f em.mdp -o em1 -c membedded.gro  -p ${prot}.top -maxwarn 3
...
gmx_sse mdrun -v -s em1 -o em1 -c after_em1.gro 

gmx_sse editconf -f after_em1.gro -o after_em1.pdb
...
gmx_sse editconf -f em1.tpr -o ref1.pdb

mv ions_in.tpr ions_in_backup.tpr

gmx_sse grompp -f em.mdp -c em_sol.gro -p ${prot}.top -o ions_in
...
gmx_sse grompp -v -f em -o em2 -c ions_mod.gro -p ${prot}.top
...
gmx_sse mdrun -v -s em2 -o em2  -c after_em2

gmx_sse editconf -f after_em2.gro -o after_em2.pdb

gmx_sse editconf -f em2.tpr -o ref2.pdb

#
## 10;; Prepare for equilibration
#

#gmx_sse grompp -f pr_POPC_berger_test.mdp -c after_em2.gro -p ${prot}.top -o pr_test.tpr
#gmx_sse grompp -f pr_POPC_berger.mdp -c after_em2.gro -p ${prot}.top -o pr_full.tpr
# pr_POPC_berger.mdp is fucked! No pressure!

gmx_sse grompp -f pr_GPU_POPC_test.mdp -c after_em2.gro -p ${prot}.top -o pr_test.tpr
gmx_sse grompp -f pr_GPU_POPC.mdp -c after_em2.gro -p ${prot}.top -o pr_full.tpr

mv pr_test.gro pr_test_backup.gro

gmx_sse mdrun -v -deffnm pr_test
...
if [ ! -f pr_test.gro ]; then
        ...
        with: gmx_sse mdrun -v -deffnm pr_full >& out &
        ##########################################"
        exit 0
fi
